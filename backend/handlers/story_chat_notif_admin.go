...
func BanUser(c *gin.Context) {
	// Соҳиби барнома ва админҳо бан намешаванд.
	tag, err := db.Pool.Exec(context.Background(),
		`UPDATE users SET banned=TRUE
		 WHERE id=$1 AND username<>'raonson' AND COALESCE(role,'user')<>'admin'`, c.Param("id"))
...
func AdminDeleteUser(c *gin.Context) {
	...
	// Ҳамон нест кардани пурра, ки корбар худаш мекунад (ниг. deleteAccount).
