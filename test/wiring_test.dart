...
void main() {
  ...
  group('бақайдгирӣ бе SMS баста намешавад', () {
    // Пеш бе рамзи 6-рақама аз қадами якум гузаштан ҒАЙРИИМКОН буд.
    // Рамз аз SMS меомад, SMS Twilio мехост, Twilio танзим нашуда
    // буд — пас ҲЕҶ КАС бақайд шуда наметавонист.
    late String src;
    setUpAll(() => src =
        _read('lib/auth/register/register_flow_screen.dart'));

    test('ҳангоми 502 бақайдгирӣ давом мекунад', () {
      expect(src, contains('e.status == 502'),
          reason: 'бе канали танзимшуда бақайдгирӣ баста мемонад');
      // Маҳз `_goto(1)` — гузаштан ба қадами дуюм.
      final i = src.indexOf('e.status == 502');
      expect(src.substring(i, i + 200), contains('_goto(1)'),
          reason: '502 гирифта мешавад, вале корбар пеш намеравад');
    });

    test('хатои ДИГАР ҳанӯз нишон дода мешавад', () {
      // Масалан рақами нодуруст ё интернет нест — инҳо набояд
      // хомӯш гузаранд, вагарна корбар бе интернет «бақайд» мешуду
      // баъд ҳеҷ чиз кор намекард.
      expect(src, contains("_error = e.message ?? 'Рамз фиристода нашуд'"));
      expect(src, contains("_error = 'Рамз фиристода нашуд'"));
    });

    test('рамз аз байн НАРАФТ — ҳамон ҷояш аст', () {
      // Ҳамон рӯзе, ки TWILIO_* гузошта шавад, он худкор боз
      // талаб мешавад. Дар барнома ҳеҷ чиз иваз кардан лозим нест.
      expect(src, contains('_showOtpDialog'));
      expect(src, contains('_phoneVerified = true'));
    });
  });
}
