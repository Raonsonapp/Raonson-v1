...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  /// Тасдиқи телефон — вале он бақайдгириро БАСТА НАМЕКУНАД.
  ///
  /// ══════════════════════════════════════════════════════════════
  ///  Чаро чунин.
  ///
  ...
  ///  буд — пас ҲЕҶ КАС бақайд шуда наметавонист. Дарвоза баста
  ///  буд ва калид вуҷуд надошт.
  ///
  ///  Акнун: агар сервер гӯяд, ки рамз фиристода НАШУД (502 —
  ///  ягон канал танзим нашудааст), бақайдгирӣ бе тасдиқи телефон
  ///  давом мекунад.
  ///
  ///  Ин рамзро НАБУРД — он ҷои худро нигоҳ дошт. Ҳамон рӯзе, ки
  ///  дар сервер TWILIO_SID/TOKEN/FROM (ё TELEGRAM_GATEWAY_TOKEN)
  ///  гузошта шавад, рамз ХУДКОР боз талаб мешавад — дар барнома
  ///  ҳеҷ чиз иваз кардан лозим нест.
  ///
  ///  Хатои ШАБАКА бошад (интернет нест) — ин дигар аст ва нишон
  ///  дода мешавад: вагарна корбар бе интернет «бақайд» мешуду
  ...
  Future<void> _sendPhoneOtp(String phone) async {
    ...
    try {
      ...
      setState(() => _loading = false);
      if (e.status == 502) {
        // Сервер рамзро фиристода НАТАВОНИСТ — ягон канал танзим
        // нашудааст. Ин айби корбар нест, пас ӯро наист.
        _goto(1);
        return;
      }
      // Ҳар хатои дигар (масалан рақами нодуруст) — нишон медиҳем.
      setState(() => _error = e.message ?? 'Рамз фиристода нашуд');
    } catch (_) {
