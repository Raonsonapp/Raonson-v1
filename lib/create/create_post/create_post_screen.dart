...
class _CreatePostScreenState extends State<CreatePostScreen> {
  ...
  Future<void> _publish() async {
    ...
    try {
      ...
      // ── 2. POST /posts/ (trailing slash муҳим!) ──────────────────
      setState(() { _status = 'Пост сохта мешавад...'; _progress = 0.85; });

      final postRes = await http.post(
        Uri.parse('${AppConfig.apiBaseUrl}/posts/'), // ← /posts/ бо slash
        headers: {
      ...
      if (postRes.statusCode >= 400) {
        ...
        try { err = jsonDecode(postRes.body) as Map<String, dynamic>; } catch (_) {}
        throw Exception(
          'Пост сохта нашуд (${postRes.statusCode}): ${err['message'] ?? postRes.body}',
        );
      }
